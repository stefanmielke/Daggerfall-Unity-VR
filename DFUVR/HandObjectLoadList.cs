...
namespace DFUVR
{
    internal class HandObjectLoadList : MonoBehaviour
    {
        ...
        public static void LoadAllHandObjects(Dictionary<WeaponTypes, HandObject> handObjectDictionary)
        {
            ...
            foreach (var handObjectLoad in handObjectLoadList)
            {
                ...
                var gameObject = Instantiate(currentBundle.LoadAsset<GameObject>(handObjectLoad.assetName));
                if (gameObject == null)
                ...
                if (handObjectLoad.weaponTypes != null)
                {
                    foreach (var type in handObjectLoad.weaponTypes)
                        handObjectDictionary.Add(type, new HandObject(gameObject, handObjectLoad.sheatedPositionOffset, handObjectLoad.sheatedRotationOffset, handObjectLoad.unsheatedPositionOffset, handObjectLoad.unsheatedRotationOffset, handObjectLoad.renderUnsheated));
                }
